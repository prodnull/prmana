...
Hardware signer behavior is controlled by `signer.yaml` at one of these locations (first found wins):

- **Linux:** `~/.config/prmana/signer.yaml` or `/etc/prmana/signer.yaml`
- **macOS:** `~/Library/Application Support/com.prmana.prmana/signer.yaml` or `/etc/prmana/signer.yaml`

```yaml
yubikey:
  # Path to the PKCS#11 provider library.
  # Auto-detected at runtime from well-known locations:
  #   Linux: /usr/lib/libykcs11.so, /usr/lib/x86_64-linux-gnu/libykcs11.so
  #   macOS: /opt/homebrew/lib/libykcs11.dylib, /usr/local/lib/libykcs11.dylib
  # Override only if your library is in a non-standard location.
  # pkcs11_library: /custom/path/libykcs11.so
