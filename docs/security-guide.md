...
The project supports step-up authentication for `sudo` and similar privileged operations via OAuth 2.0 Device Authorization Grant (RFC 8628).

Current posture:

- agent initiates device flow for step-up approval
- PAM can require step-up before granting elevated privileges
