...
/**
 * Training Video: prmana Authentication Flows
 *
...
test.describe('prmana Training Videos', () => {
  ...
  test('Demo 1: Keycloak Admin Console', async ({ page }) => {
    ...
    await showCaption(page, '🔐 Keycloak Admin Console - Configure OIDC authentication for prmana');
    ...
    await page.waitForTimeout(500);
    await page.click('text=prmana-test');
    await showCaption(page, '🏢 prmana-test realm - Users, clients, and authentication policies');

    await page.getByRole('link', { name: 'Clients' }).click();
    await showCaption(page, '📋 OIDC Clients - "prmana" client handles SSH and sudo authentication', 4000);
  ...
  test('Demo 3: Sudo Step-Up with Device Flow', async ({ page }) => {
    ...
    await showCaption(page, '🔒 prmana PAM module intercepts sudo - requires fresh authentication');
    ...
    const deviceResponse = await page.request.post(
      ...
      { form: { client_id: 'prmana', client_secret: 'prmana-test-secret', scope: 'openid' } }
    );
  ...
  test('Demo 4: Policy Configuration', async ({ page }) => {
    ...
    const policyContent = `
<span class="output"># /etc/prmana/policy.yaml</span>
...
`.trim();
    ...
    await page.goto('about:blank');
    await page.setContent(getTerminalHTML('cat /etc/prmana/policy.yaml'));
    await page.waitForTimeout(500);
...
`.trim();
    ...
    await page.goto('about:blank');
    await page.setContent(getTerminalHTML('tail -f /var/log/prmana/audit.json | jq'));
    await page.waitForTimeout(500);
  ...
  test('Demo 6: Realm Security Settings', async ({ page }) => {
    ...
    await page.click('[data-testid="realmSelectorToggle"]');
    await page.click('text=prmana-test');
    await showCaption(page, '🏢 Configuring prmana-test realm - Security policies for SSH and sudo');
    ...
    await showCaption(page, '✅ Keycloak provides enterprise-grade security controls for prmana', 4000);
    await clearCaption(page);
  });
});
