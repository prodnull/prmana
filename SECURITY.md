...
4. **Minimal Privileges (PAM module + agent daemon)**: The PAM module runs within the sshd process and the agent daemon runs as a dedicated service user with only the privileges required for token acquisition and DPoP proof generation.
...
- **GitHub**: [Create private security advisory](https://github.com/prodnull/prmana/security/advisories/new)
