...
if [[ -n "$ACCESS_TOKEN" ]] && [[ "$ACCESS_TOKEN" == *.*.* ]]; then
    ...
    # (https://prmana.dev/preferred_username).  Accept any of these —
...
print(d.get(ns + 'preferred_username', '') or d.get('preferred_username', '') or d.get('https://prmana.dev/preferred_username', ''))
" 2>/dev/null || echo "")
