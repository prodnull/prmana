...
/// Well-known PKCS#11 library paths for ykcs11, tried in order at runtime.
#[cfg(target_os = "linux")]
const PKCS11_SEARCH_PATHS: &[&str] = &[
...
const PKCS11_SEARCH_PATHS: &[&str] = &["libykcs11.so"];
...
///
/// Priority: explicit config > first existing path from search list.
fn pkcs11_path(config: &SignerConfig) -> String {
    ...
    for candidate in PKCS11_SEARCH_PATHS {
        if std::path::Path::new(candidate).exists() {
            return candidate.to_string();
        }
    }
    PKCS11_SEARCH_PATHS[0].to_string()
}
        ...
        anyhow!(
            ...
            HardwareSignerError::KeyNotFound(format!(
                "No P-256 key found in YubiKey PIV slot (key ID {key_id:#x}). \
                 Use `prmana-agent provision --signer yubikey:9a` to generate one."
            ))
        )
    })?;
