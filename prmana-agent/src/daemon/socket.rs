...
async fn handle_step_up(
    ...
    use pam_prmana::ciba::{build_binding_message, CibaClient, ACR_PHR};
    use pam_prmana::oidc::OidcDiscovery;
    ...
    let bc_auth: pam_prmana::ciba::BackchannelAuthResponse = match bc_response.json().await {
        Ok(r) => r,
        Err(e) => {
            return AgentResponse::error(
    ...
    // older PAM versions that do not set PRMANA_SESSION_ID before exec-ing sudo).
    tracing::info!(
        target: "prmana_audit",
...
pub(crate) async fn poll_ciba(
    ...
    use pam_prmana::ciba::{parse_ciba_error, validate_acr, CibaError, CibaTokenResponse};
