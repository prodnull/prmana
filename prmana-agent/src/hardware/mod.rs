...
/// `~/.config/prmana/signer.yaml` or `/etc/prmana/signer.yaml`.
...
impl SignerConfig {
    ...
    ///
    /// Search order:
    /// 1. `~/.config/prmana/signer.yaml`  (via `directories::ProjectDirs`)
    /// 2. `/etc/prmana/signer.yaml`
    ...
    pub fn load() -> Self {
        ...
        let user_path = directories::ProjectDirs::from("com", "prmana", "prmana")
    ...
    if signer_spec == "software" || signer_spec.is_empty() {
        anyhow::bail!(
            "Software signer auto-generates at login. No provisioning needed.\n\
             Run `prmana-agent login` to authenticate with a software key."
        );
    }
