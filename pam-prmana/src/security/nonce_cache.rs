...
/// under concurrent load. Directory: `/run/prmana/nonce/`.
///
/// Override with `PRMANA_NONCE_DIR` for testing.
pub fn global_nonce_store() -> &'static FsAtomicStore {
    static STORE: Lazy<FsAtomicStore> =
        Lazy::new(|| FsAtomicStore::new("/run/prmana/nonce", "PRMANA_NONCE_DIR"));
    &STORE
}
