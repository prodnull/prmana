...
//! - `OIDC_CLIENT_ID` (optional): Expected audience, defaults to "prmana"
...
//! - `PRMANA_TEST_MODE` (optional): Enable test mode (allows token from OIDC_TOKEN env var)
...
impl PamServiceModule for PamUnixOidc {
        ...
        match auth_result {
            Ok(result) => {
                ...
                if let Ok(policy) = PolicyConfig::from_env() {
                    if policy.introspection.enabled {
                        let client_id = std::env::var("OIDC_CLIENT_ID")
                            .unwrap_or_else(|_| "prmana".to_string());
                ...
                if let Err(e) = pamh.putenv(&format!(
                    ...
                    tracing::warn!(error = ?e, "Failed to set PRMANA_TOKEN_JTI in PAM env");
                }
                if let Err(e) = pamh.putenv(&format!("PRMANA_TOKEN_EXP={}", result.token_exp)) {
                    tracing::warn!(error = ?e, "Failed to set PRMANA_TOKEN_EXP in PAM env");
                }
    ...
    fn open_session(pamh: Pam, _flags: PamFlags, _args: Vec<String>) -> PamError {
        ...
        let session_id = match pamh.getenv("PRMANA_SESSION_ID") {
            Ok(Some(s)) => s.to_string_lossy().to_string(),
            Ok(None) => {
                tracing::warn!("PRMANA_SESSION_ID absent in PAM env; skipping session record");
                return PamError::SUCCESS;
            }
            Err(e) => {
                tracing::warn!(error = ?e, "Failed to read PRMANA_SESSION_ID from PAM env");
                return PamError::SUCCESS;
            }
        };
    ...
    fn close_session(pamh: Pam, _flags: PamFlags, _args: Vec<String>) -> PamError {
        ...
        let session_id = match pamh.getenv("PRMANA_SESSION_ID") {
            Ok(Some(s)) => s.to_string_lossy().to_string(),
            Ok(None) => {
                tracing::warn!("PRMANA_SESSION_ID absent in PAM env at session close");
                return PamError::SUCCESS;
            }
            Err(e) => {
                tracing::warn!(error = ?e, "Failed to read PRMANA_SESSION_ID from PAM env at close");
                return PamError::SUCCESS;
            }
        };
...
/// CLI: `$XDG_RUNTIME_DIR/prmana-agent.sock` falling back to
/// `/run/user/0/prmana-agent.sock` (root).
...
fn get_auth_token(pamh: &Pam, test_mode: bool) -> Option<SecretString> {
    ...
    if test_mode {
        ...
        if let Ok(token) = std::env::var("OIDC_TOKEN") {
            if is_jwt(&token) {
                eprintln!(
                    "prmana: SECURITY NOTICE: Accepting token from OIDC_TOKEN environment variable (test mode)."
                );
    ...
    if is_pam_env_token_enabled() {
        ...
        eprintln!(
            "prmana: WARNING: PRMANA_ACCEPT_PAM_ENV is enabled. \
        ...
        if let Ok(Some(token)) = pamh.getenv("OIDC_TOKEN") {
            ...
            if is_jwt(&token_str) {
                eprintln!(
                    "prmana: SECURITY NOTICE: Accepting token from PAM environment variable. \
    ...
    if let Ok(Some(token)) = pamh.conv(Some("OIDC Token: "), PamMsgStyle::PROMPT_ECHO_OFF) {
        ...
        if token_str.starts_with("eyJ") && token_str.len() > 500 {
            eprintln!(
                "prmana: Warning: Token may be truncated ({} bytes). \
...
mod tests {
    ...
    // =========================================================================
    // Security tests for PRMANA_TEST_MODE gating
    // =========================================================================
    ...
    fn test_test_mode_not_enabled_with_other_values() {
        ...
        for value in &["yes", "TRUE", "True", "enabled", "on", "0", "false", ""] {
            std::env::set_var("PRMANA_TEST_MODE", value);
            assert!(
                !is_test_mode_enabled(),
    ...
    // =========================================================================
    // Security tests for PRMANA_ACCEPT_PAM_ENV gating
    // =========================================================================
    ...
    fn test_pam_env_token_enabled_with_true() {
        let _guard = ENV_MUTEX.lock();
        std::env::set_var("PRMANA_ACCEPT_PAM_ENV", "true");
    ...
    fn test_pam_env_token_enabled_with_1() {
        let _guard = ENV_MUTEX.lock();
        std::env::set_var("PRMANA_ACCEPT_PAM_ENV", "1");
        assert!(is_pam_env_token_enabled());
        std::env::remove_var("PRMANA_ACCEPT_PAM_ENV");
    }
    ...
    fn test_pam_env_token_not_enabled_with_other_values() {
        ...
        for value in &["yes", "TRUE", "True", "enabled", "on", "0", "false", ""] {
            std::env::set_var("PRMANA_ACCEPT_PAM_ENV", value);
            assert!(
                !is_pam_env_token_enabled(),
    ...
    fn test_security_invariant_explicit_opt_in_required() {
        ...
        std::env::set_var("PRMANA_TEST_MODE", "false");
        std::env::set_var("PRMANA_ACCEPT_PAM_ENV", "no");
    ...
    fn test_dpop_mode_default_is_strict_without_policy_file() {
        ...
        // (default policy path /etc/prmana/policy.yaml absent).
        // Our authenticate() code maps this to EnforcementMode::Strict (safe default).
        std::env::remove_var("PRMANA_POLICY_FILE");
    ...
    // Requires test-mode: PRMANA_POLICY_YAML is gated behind #[cfg(feature = "test-mode")]
    ...
    fn test_dpop_mode_from_inline_yaml_disabled() {
        ...
        std::env::set_var("PRMANA_POLICY_YAML", yaml);
    ...
    // not PRMANA_AGENT_SOCKET env var, so the test socket can't be reached.
    ...
    fn test_notify_agent_session_closed_sends_newline_framed_json() {
        ...
        let handle = std::thread::spawn(move || {
            let _guard = ENV_MUTEX.lock();
            std::env::set_var("PRMANA_AGENT_SOCKET", &sock_path_str);
            notify_agent_session_closed("test-session-id-123");
            std::env::remove_var("PRMANA_AGENT_SOCKET");
        });
    ...
    fn test_extract_iss_for_routing_parses_issuer() {
        ...
        let payload = URL_SAFE_NO_PAD.encode(
            format!(r#"{{"iss":"https://idp.example.com","sub":"alice","aud":"prmana","exp":{exp},"iat":{now}}}"#,
                exp = now + 3600)
                .as_bytes(),
        );
    ...
    fn test_multi_issuer_dispatch_unknown_issuer_via_authenticate_multi_issuer() {
        ...
        let header = URL_SAFE_NO_PAD.encode(r#"{"alg":"ES256","typ":"JWT"}"#.as_bytes());
        let payload = URL_SAFE_NO_PAD.encode(
            format!(r#"{{"iss":"https://unknown.example.com","sub":"x","aud":"prmana","exp":{exp},"iat":{now}}}"#)
                .as_bytes(),
        );
        let token = format!("{header}.{payload}.dummysig");
