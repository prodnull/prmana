...
//! Environment variable override pattern: PRMANA_SECURITY_MODES__JTI_ENFORCEMENT
...
pub struct AcrMappingConfig {
    ...
    /// E.g. `{"urn:idp:mfa": "urn:prmana:acr:mfa"}`.
    pub mappings: HashMap<String, String>,
...
pub struct IssuerConfig {
    ...
    /// OAuth client ID for this issuer. Default: `"prmana"`.
    ...
    /// Application ID URI (e.g. `api://prmana`) that differs from the client ID GUID.
...
///
/// Persisted to disk (`/run/prmana/issuer-health/<sha256-prefix>.json`) so that
...
///
/// The health directory is configurable via `PRMANA_HEALTH_DIR` for testing,
/// defaulting to `/run/prmana/issuer-health/`.
///
...
impl IssuerHealthManager {
    ...
    ///
    /// The health directory is resolved from `PRMANA_HEALTH_DIR` env var
    /// (for testing) or defaults to `/run/prmana/issuer-health/`.
    pub fn new() -> Self {
        let health_dir = std::env::var("PRMANA_HEALTH_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from("/run/prmana/issuer-health"));
        Self { health_dir }
    }
...
impl Default for SessionConfig {
    fn default() -> Self {
        Self {
            session_dir: "/run/prmana/sessions".to_string(),
            token_refresh_threshold_percent: 80,
        }
    }
}
...
pub struct BreakGlassConfig {
    ...
    /// and verifies offline against /etc/prmana/break-glass-otp.json.
...
/// - `PRMANA_TIMEOUTS__CLOCK_SKEW_FUTURE_SECS=10`
/// - `PRMANA_TIMEOUTS__CLOCK_SKEW_STALENESS_SECS=90`
#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(default)]
...
///
/// Environment variable override pattern (double-underscore nesting):
/// - `PRMANA_AUDIT__LOG_FILE=/var/log/prmana-audit.log`
/// - `PRMANA_AUDIT__SYSLOG_ENABLED=true`
/// - `PRMANA_AUDIT__HMAC_KEY_FILE=/etc/prmana/audit-hmac.key`
#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(default)]
pub struct AuditConfig {
    /// Path to the dedicated audit log file. JSON-lines format, one event per line.
    /// Set to empty string to disable file logging.
    /// Default: `/var/log/prmana-audit.log`
    pub log_file: String,
    ...
    /// When empty, falls back to `PRMANA_AUDIT_HMAC_KEY` environment variable.
...
impl Default for AuditConfig {
    fn default() -> Self {
        Self {
            log_file: "/var/log/prmana-audit.log".to_string(),
            syslog_enabled: true,
...
///
/// Loaded via figment from `/etc/prmana/policy.yaml` with optional
/// environment-variable overrides (prefix `PRMANA_`, double-underscore nesting).
///
...
impl PolicyConfig {
    ...
    pub fn load() -> Result<Self, PolicyError> {
        Self::load_from("/etc/prmana/policy.yaml")
    }
    ...
    /// - `PRMANA_SECURITY_MODES__JTI_ENFORCEMENT=strict`
    /// - `PRMANA_SECURITY_MODES__DPOP_REQUIRED=warn`
    /// - `PRMANA_CACHE__JTI_MAX_ENTRIES=50000`
    ///
    ...
    /// `PRMANA_*` vars (e.g. `PRMANA_TEST_MODE`) from causing spurious
    /// config-parse errors.
    pub fn load_from<P: AsRef<Path>>(path: P) -> Result<Self, PolicyError> {
        ...
        let config: PolicyConfig = Figment::from(Serialized::defaults(PolicyConfig::default()))
            ...
            .merge(Env::prefixed("PRMANA_").split("__").only(&[
                "security_modes",
                "cache",
                "identity",
                "introspection",
    ...
    pub fn from_env() -> Result<Self, PolicyError> {
        ...
        // builds. In production, environment injection of PRMANA_POLICY_YAML cannot
        ...
        #[cfg(feature = "test-mode")]
        if let Ok(yaml) = std::env::var("PRMANA_POLICY_YAML") {
            let config: PolicyConfig = Figment::from(Serialized::defaults(PolicyConfig::default()))
...
"#;
    ...
    fn test_env_var_override_jti_enforcement() {
        // PRMANA_SECURITY_MODES__JTI_ENFORCEMENT=strict overrides YAML
        let yaml = r#"
security_modes:
  jti_enforcement: warn
"#;
        // Safety: test-only env var manipulation; tests run sequentially within this module
        std::env::set_var("PRMANA_SECURITY_MODES__JTI_ENFORCEMENT", "strict");
        let result: Result<PolicyConfig, _> =
            Figment::from(Serialized::defaults(PolicyConfig::default()))
                .merge(Yaml::string(yaml))
                .merge(Env::prefixed("PRMANA_").split("__"))
                .extract();
        std::env::remove_var("PRMANA_SECURITY_MODES__JTI_ENFORCEMENT");
    ...
    fn test_unknown_env_vars_do_not_break_load() {
        // PRMANA_TEST_MODE is not a PolicyConfig field — figment must not error.
        // We use `.only()` filter on the env provider to prevent unknown key mapping.
        std::env::set_var("PRMANA_TEST_MODE", "true");
        let result: Result<PolicyConfig, _> =
            Figment::from(Serialized::defaults(PolicyConfig::default()))
                .merge(Env::prefixed("PRMANA_").split("__").only(&[
                    "security_modes",
                ...
                .extract();
        std::env::remove_var("PRMANA_TEST_MODE");
...
"#;
    ...
    fn test_session_config_defaults() {
        ...
        assert_eq!(config.session_dir, "/run/prmana/sessions");
...
"#;
        ...
        assert_eq!(policy.session.session_dir, "/run/prmana/sessions");
...
"#;
    ...
    fn test_pam_timeouts_env_override() {
        // PRMANA_TIMEOUTS__CLOCK_SKEW_FUTURE_SECS overrides default via figment env provider.
        // Uses bare split("__") (no .only()) to match production load_from() behavior.
        std::env::set_var("PRMANA_TIMEOUTS__CLOCK_SKEW_FUTURE_SECS", "15");
        let result: Result<PolicyConfig, _> =
            Figment::from(Serialized::defaults(PolicyConfig::default()))
                .merge(Env::prefixed("PRMANA_").split("__"))
                .extract();
        std::env::remove_var("PRMANA_TIMEOUTS__CLOCK_SKEW_FUTURE_SECS");
...
"#;
    ...
    #[test]
    fn policy_without_sudo_section_parses_to_defaults() {
        let yaml = r#"
issuers:
...
"#;
        ...
        assert!(config.sudo.step_up_required);
        assert_eq!(config.sudo.challenge_timeout, 120);
        assert!(config.sudo.commands.is_empty());
    }

    #[test]
    fn oss_example_policy_without_sudo_parses_cleanly() {
        let example = std::fs::read_to_string(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../examples/policy.yaml"),
        )
        .unwrap();
        let config: PolicyConfig = serde_yaml::from_str(&example).unwrap();
        assert!(config.ssh_login.require_oidc);
        assert!(config.break_glass.enabled);
    }
}
