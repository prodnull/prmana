...
pub fn authenticate_multi_issuer(
    ...
    // their SIEM. Target `prmana_audit` ensures this appears in the audit stream.
...
///
/// # Test Mode
/// When compiled with `--features test-mode` AND `PRMANA_TEST_MODE` env var is set,
...
mod tests {
    ...
    fn test_secure_session_id_format() {
        ...
        // New format: prmana-{timestamp_hex}-{32_char_random_hex}
    ...
    fn test_dpop_auth_config_from_env() {
        ...
        std::env::set_var("PRMANA_DPOP_MAX_AGE", "120");
        std::env::set_var("PRMANA_DPOP_REQUIRE_NONCE", "1");
        ...
        // Clean up
        std::env::remove_var("PRMANA_DPOP_MAX_AGE");
        std::env::remove_var("PRMANA_DPOP_REQUIRE_NONCE");
    ...
    fn test_auth_result_mapped_from_field_exists() {
        ...
        let result = AuthResult {
            ...
            uid: 1000,
            gid: 1000,
            session_id: "prmana-abc-0123456789abcdef".to_string(),
            token_jti: None,
            token_acr: None,
...
"#;
    ...
    // because tests run with PRMANA_TEST_MODE=1 which uses new_insecure_for_testing).
    // Format: <base64url(header)>.<base64url(payload)>.<dummy_sig>
    //
    // ENV_MUTEX serializes tests that manipulate PRMANA_TEST_MODE to prevent
    ...
    fn test_apply_per_issuer_dpop_strict_rejects_missing_attestation() {
        let claims = crate::oidc::token::TokenClaims {
            ...
            aud: crate::oidc::token::StringOrVec::String("prmana".into()),
            exp: 9_999_999_999,
            iat: 1_700_000_000,
            auth_time: None,
            acr: None,
    ...
    fn make_test_jwt_no_preferred_username(iss: &str, sub: &str, jti: Option<&str>) -> String {
        ...
        let payload = format!(
            r#"{{"iss":"{iss}","sub":"{sub}","aud":"prmana","exp":{exp},"iat":{now}{jti_field}}}"#
        );
    ...
    fn make_test_jwt_with_email_no_preferred_username(
        ...
        let payload = format!(
            r#"{{"iss":"{iss}","sub":"{sub}","aud":"prmana","exp":{exp},"iat":{now},"email":"{email}"{jti_field}}}"#
        );
        let h = URL_SAFE_NO_PAD.encode(header.as_bytes());
        let p = URL_SAFE_NO_PAD.encode(payload.as_bytes());
        format!("{h}.{p}.dummysig")
